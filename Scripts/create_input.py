...
def join_tiles_in_map(list_chars_columns,method="1"):
    if method=="1":
        return "".join(list_chars_columns)
...
def data_one_map_tiles(args, logger):
    ...
    list_chars=join_tiles_in_map(list_chars_columns)
    ...
    logger.info('----- Generation seed:')
    logger.info( [sentence[i:i+column_length] for i in range(0, len(sentence), column_length)] )
    logger.info('------------')
    test = np.zeros((1, maxlen))
    for t, word in enumerate(sentence):
        test[0, t ] = char_indices[word]

    return X, y, test, char_indices, indices_char


def create_dataloaders(args, logger):
    X, y, test, word_indices, indices_word = data_one_map_tiles(args, logger)
    ...
    return dataloader, test_dataloader, (word_indices, indices_word)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='../logs/test.log',
                            ...
                            datefmt='%H:%M:%S',
                            level=logging.INFO)
    dataloader, test_dataloader, _ = create_dataloaders(args, logging.getLogger('Test'))
